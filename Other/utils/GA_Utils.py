...
def Creat_child(moead,index):
    configurations = moead.configurations
    # 创建一个个体
    child = configurations[index]
    return child


def Creat_Pop(moead, init_samples):
...
def generate_next(moead, gen, wi, p0, p1, p2, probabilities, parato_frontier, n_of_synthesis, pareto_frontier_exhaustive, adrs_evolution,time_evolution, all_time):
    ...
    if not MOEAD_Utils.check_isSynthesis(moead, n_p0):
        # 更新ADRS
        MOEAD_Utils.adrs_undate(moead, n_p0, parato_frontier, pareto_frontier_exhaustive, n_of_synthesis, adrs_evolution,time_evolution, all_time)
    ...
    if not MOEAD_Utils.check_isSynthesis(moead, n_p1):
        MOEAD_Utils.adrs_undate(moead, n_p1, parato_frontier, pareto_frontier_exhaustive, n_of_synthesis,
                                adrs_evolution,time_evolution, all_time)
...
def envolution(moead):
    ...
    for i in unique_configurations:
        unique_point.append(
            DSpoint(moead.Test_fun.latency(i, moead.entire_ds), moead.Test_fun.area(i, moead.entire_ds),moead.Test_fun.time(i, moead.entire_ds), i))
        all_time += moead.Test_fun.time(i, moead.entire_ds)
    ...
    for gen in range(moead.max_gen):
        ...
        for pi, p in enumerate(moead.Pop):
            ...
            Xl = moead.Pop[il]
            # 进化下一代个体。基于自身Xi+邻居中随机选择的2个Xk，Xl 还考虑gen 去进化下一代
            Y = generate_next(moead, gen, pi, Xi, Xk, Xl, probabilities, pareto_frontier, n_of_synthesis, pareto_frontier_exhaustive, adrs_evolution,time_evolution, all_time)
            # 计算当前Xi，不进化前的切比雪夫距离
    ...
    return moead.EP_X_ID, adrs_evolution, time_evolution,n_of_synthesis, initial_sampling_size
